let DetailData = {
  web: [
  {
    id: 1,
    title: "Ciao Kombucha",
    description: "Refonte de l'expérience utilisateur (UX) et de l'interface (UI) du site web de la marque Ciao Kombucha afin de proposer une plateforme qui reflète au mieux l'énergie et le style de la marque.",
    type: "projet personnel",
    secteurs :["design", "développement"],
    infos :["Figma", "HTML", "CSS", "Javascript"],
    images: [
        "./asset/ciaoKombucha/ciaoKombucha.webp", "./asset/ciaoKombucha/menthMainT.jpg", "./asset/ciaoKombucha/menthT.jpg", "./asset/ciaoKombucha/dragonSecD.jpg", "./asset/ciaoKombucha/infuD.jpg", "./asset/ciaoKombucha/fermT.jpg", "./asset/ciaoKombucha/filtreT.jpg"
    ],
    link: "https://www.behance.net/gallery/245125823/Ciao-Kombucha-redisign-web-UX-UI?platform=direct"
  },
  
  {
    id: 2,
    title: "Ombra Systems",
    description: "Conception, intégration et développement de A à Z d'un site 'One Page' d'une startup. Le projet consistait à utiliser un kit de composants d'interface pour construire la page, puis à l'alimenter via un jeu de données.",
    type: "projet étudiant",
    secteurs :["design", "développement"],
    infos :["HTML", "CSS", "Javascript"],
    images: [
        "./asset/ombraSystems/ombraSystems.webp", "./asset/ombraSystems/T1.jpg", "./asset/ombraSystems/T2.jpg", "./asset/ombraSystems/D1.jpg"
    ],
    link: "https://etchegoyen-sae105.mmi-limoges.fr/OmbraSystems.html"
  },

  {
    id: 3,
    title: "David Lynch",
    description: "design web d'un événement fictif organisé au cinéma d'art et d'essai Le Lido dédiée à une rétrospective sur le thème des films du réalisateur David Lynch.",
    type: "projet étudiant",
    secteurs :["design"],
    infos :["Figma"],
    images: [
        "./asset/davidLynch/davidLynch.webp", "./asset/davidLynch/blueVelvet.jpg", "./asset/davidLynch/twinPeak.jpg", "./asset/davidLynch/all.jpg"
    ],
    link: "https://www.behance.net/malo_etchegoyen"
  },

  {
    id: 4,
    title: "Moviethèque",
    description: "Plateforme de 'VOD' s'appuyant sur une base de données comprenant à la fois l'application destinée aux utilisateurs finaux et le back-office sécurisé pour les administrateurs.",
    type: "projet étudiant",
    secteurs :["design", "développement"],
    infos :["HTML", "CSS", "JavaScript", "PHP", "MySQL"],
    images: [
        "./asset/movietheque/movietheque.webp", "./asset/movietheque/T1.jpg", "./asset/movietheque/T2.jpg", "./asset/movietheque/recherche.jpg"
    ],
    link: "https://etchegoyen-sae203.mmi-limoges.fr/app/index.html"
  }
],


  graphic: [
    {
    id: 1,
    title: "Affiche mediévale",
    description: "Création d'une composition graphique illustrant des thèmes variés",
    type: "projet personnel",
    secteurs :["design"],
    infos :["Photoshop"],
    images: [
        "./asset/medieval/dedication.jpg", "./asset/medieval/bravery.jpg"
    ],
    link: "https://www.instagram.com/reel/DSx8JZ3DNhd/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA=="

  },
    {
    id: 2,
    title: "Stickers Skills",
    description: "Stickers représentant les soft skills et hard skills dans le style de Mike Karolos",
    type: "projet étudiant",
    secteurs :["design"],
    infos :["Illustrator"],
    images: [
      "./asset/stickers/illustrator/designWeb.png", "./asset/stickers/illustrator/creatif.png", "./asset/stickers/illustrator/dessin.png", "./asset/stickers/illustrator/ecoute.png", "./asset/stickers/illustrator/adaptabilité.png","./asset/stickers/illustrator/empathie.png", "./asset/stickers/illustrator/avatar.png"
    ],
    link: ""
  },
      {
    id: 3,
    title: "Happiness",
    description: "Visuel surréaliste et vibrant autour du thème de la joie",
    type: "projet personnel",
    secteurs :["design"],
    infos :["Photoshop"],
    images: [
        "./asset/happiness.jpg"
    ],
    link: "https://www.instagram.com/reel/DSsUtGpDG7m/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA=="

  },
      {
    id: 4,
    title: "Covers Albums de Billie Eilish",
    description: "Conception graphique et réinterprétation visuelle des pochettes d'albums de l'artiste Billie Eilish.",
    type: "projet personnel",
    secteurs :["design"],
    infos :["Photoshop"],
    images: [
        "./asset/BillieEilish/HTE.jpg", "./asset/BillieEilish/HMHAS.jpg"
    ],
    link: "https://www.instagram.com/reel/DSnC0acDMso/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA=="

  },
      {
    id: 5,
    title: "Stickers Skills",
    description: "Sticker representant les soft skill et hard skill dans le style de Pierre-Paul Pariseau.",
    type: "projet étudiant",
    secteurs :["design"],
    infos :["Photoshop"],
    images: [
        "./asset/stickers/photoshop/adaptabilité.jpg", "./asset/stickers/photoshop/dessin.jpg", "./asset/stickers/photoshop/creatif.jpg", "./asset/stickers/photoshop/perfectionniste.jpg", "./asset/stickers/photoshop/designWeb.jpg", "./asset/stickers/photoshop/ecoute.jpg", "./asset/stickers/photoshop/empathie.jpg", "./asset/stickers/photoshop/intégrationWeb.jpg",
    ],
    link: ""
  },

  ],
  video:[
    {
      id: 1,
      title: "A Step In A Better World",
      description: "Vidéo stop-motion réalisée en groupe de 3 étudiants. L'objectif était de réaliser une vidéo afin de présenter une action concrète d'une marque, valorisant son engagement pour un monde meilleur",
      type: "projet étudiant",
      secteurs: ["production vidéo", "stop motion", "communication"],
      infos: ["Premiere Pro", "DaVinci Resolve", "Photoshop"],
      images: ["./asset/AStepInABetterWorld/cover.jpg", "./asset/AStepInABetterWorld/storyboard1.jpg", "./asset/AStepInABetterWorld/storyboard2.jpg"],
      link: "https://youtu.be/eLX5izQ_IFc?si=azprGXkD79mQjrq7"
    }
  ]
};

export{DetailData}